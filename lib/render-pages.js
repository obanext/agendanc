...
@font-face{font-family:"Avenir OBA";src:url(data:font/woff2;base64,${blackFont}) format("woff2");font-weight:900;font-style:normal}`;
  ...
  for (const branch of Object.values(data)) {
    const branchSlug = slug(branch.branch);
    const landscapeDir = path.join(landscapeTemp, branchSlug);
    const portraitDir = path.join(portraitTemp, branchSlug);

    fs.mkdirSync(landscapeDir, { recursive: true });
    fs.mkdirSync(portraitDir, { recursive: true });

    fs.writeFileSync(path.join(landscapeDir, "index.html"), renderLandscape(branch, fontCss));
    fs.writeFileSync(path.join(portraitDir, "index.html"), renderPortrait(branch, fontCss));
  }
