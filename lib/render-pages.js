...
@font-face{font-family:"Avenir OBA";src:url(data:font/woff2;base64,${blackFont}) format("woff2");font-weight:900;font-style:normal}`;
  ...
  for (const branch of Object.values(data)) {
    ...
    fs.writeFileSync(path.join(landscapeTemp, `${branchSlug}.html`), renderLandscape(branch, fontCss));
    fs.writeFileSync(path.join(portraitTemp, `${branchSlug}.html`), renderPortrait(branch, fontCss));
  }
